...
export async function scrapXboxGamePassWaveAnnouncement(config) {
    ...
    const mentionedReleases = (() => {
        ...
        if (opt1) return opt1?.map(string => parseUpcomingReleasesString(string)).filter(Boolean) ?? [];
        ...
        if (opt1 === res) {
            ...
            if (opt3) {
                return opt3.map(str => {
                    ...
                    if (articleData.url === 'https://news.xbox.com/en-us/2021/08/03/coming-soon-xbox-game-pass-august-2021/') {
                        console.warn(str?.match(new RegExp(GamePattern, 'mig')))
                    }
