...
* # Import Example
```
const { default: initGitHubEsModuleLoader } = await fetch("https://raw.githubusercontent.com/dddominikk/utils/refs/heads/main/lib/utils/importEsModuleFromGitHub.mjs").then(r => r.text().then(utf8 => import(`data:application/javascript;base64,${btoa(utf8)}`)));
const importEsModuleFromGitHub = await initGitHubEsModuleLoader({
    auth: 'ghp_YOUR_GH_TOKEN',
    repoPath: 'dddominikk/utils'
});

const arrayToChunks = await importEsModuleFromGitHub('lib/utils/arrayToChunks.mjs')
console.log(arrayToChunks([1, 2, 3, 4, 5, 6, 7], 3))
```
*/
